...
namespace YCS
{
    ...
    public enum EmptyLineBehavior
    {
        ...
        EndOfFile,
        /// <summary>
        /// An empty line is interpreted as Nothing.
        /// </summary>
        DoNothing,
    ...
    /// or empty lines that appear at the end of the input file.
    /// </summary>
    public enum EmptyColumnBehavior
    {
        /// <summary>
        /// Remove All Columns After First Empty Column.
        /// </summary>
        RemoveAllColumnsAfterFirstEmptyColumn,
        /// <summary>
        /// Do Nothing with Empty Column
        /// </summary>
        DoNothing,
    ...
    public class CsvFileReader : CsvFileCommon, IDisposable
    {
        ...
        private EmptyColumnBehavior EmptyColumnBehavior;
        ...
        /// <param name="emptyColumnBehavior">Determines how empty Column are handled</param>
        public CsvFileReader(Stream stream, EmptyLineBehavior emptyLineBehavior = EmptyLineBehavior.Ignore, EmptyColumnBehavior emptyColumnBehavior = EmptyColumnBehavior.RemoveAllColumnsAfterFirstEmptyColumn)
        {
        ...
        /// <param name="emptyLineBehavior">Determines how empty lines are handled</param>
        /// <param name="emptyColumnBehavior">Determines how empty Column are handled</param>
        public CsvFileReader(string path, EmptyLineBehavior emptyLineBehavior = EmptyLineBehavior.Ignore, EmptyColumnBehavior emptyColumnBehavior = EmptyColumnBehavior.RemoveAllColumnsAfterFirstEmptyColumn)
        {
        ...
        public DataTable GetDataTableFromCSV(int SkipTopRowsCount=0)
        {
            List<string> columns = new List<string>();
            DataTable dt = new DataTable();
            SkipRow(SkipTopRowsCount);

            if (ReadRow(columns))
            {
                foreach (string header in columns)
                {
                    if (_objUtility.ValidateDBNullEmptyValue(header))
                    {
                        if (EmptyColumnBehavior == EmptyColumnBehavior.RemoveAllColumnsAfterFirstEmptyColumn)
                        { break; }
                    }
        ...
        public bool ReadRow(List<string> columns)
        {
            ...
            if (CurrLine == null)
                return false;
            // Test for empty line
            if (CurrLine.Replace(",", "").Trim().Length == 0)
            {
            ...
            while (true)
            {
                ...
                if (numColumns < columns.Count)
                    columns[numColumns] = column.Trim();
                else
